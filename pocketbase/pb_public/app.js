...
const extractPbError = e => {
  if (e?.data) {
    const details = Object.values(e.data)
      .filter(v => v && typeof v === 'object' && v.message)
      .map(v => v.message);
    if (details.length) return details.join('. ');
  }
  if (e?.response?.data) {
    const details = Object.values(e.response.data)
      .filter(v => v && typeof v === 'object' && v.message)
      .map(v => v.message);
    if (details.length) return details.join('. ');
  }
  if (e?.originalError?.response) {
    try {
      const body = JSON.parse(e.originalError.response);
      if (body.data) {
        const details = Object.values(body.data)
          .filter(v => v && typeof v === 'object' && v.message)
          .map(v => v.message);
        if (details.length) return details.join('. ');
      }
...
async function ensureProfile(user) {
  try {
    ...
    const patch = {};
    const existingName = profile.displayName || profile.name || '';
    const existingPhoto = profile.photoURL || profile.avatar || null;
    if (user.name && user.name !== existingName) patch.displayName = user.name;
    if (user.avatar && user.avatar !== existingPhoto) patch.photoURL = user.avatar;
    if (!profile.color) patch.color = getUserColor(user.id);
    if (Object.keys(patch).length) {
      await pb.collection('users').update(user.id, patch);
    }
    return { ...profile, ...patch, displayName: patch.displayName || existingName, photoURL: patch.photoURL || existingPhoto };
  } catch (e) {
    if (e.status === 404) {
...
const loginWithEmail = async () => {
  ...
  try {
    ...
    console.error('[ChatNica] Login error:', e);
    showAuthError('login', extractPbError(e));
...
const registerWithEmail = async () => {
  ...
  if (pw.length < 8) return showAuthError('reg', 'Mínimo 8 caracteres.');
  ...
  try {
    ...
    console.error('[ChatNica] Register error:', e);
    console.error('[ChatNica] Error data:', JSON.stringify(e, null, 2));
    console.error('[ChatNica] Error response:', e.response);
    console.error('[ChatNica] Error data field:', e.data);
    showAuthError('reg', extractPbError(e));
...
function showScreen(name) {
  D.loadingScreen.style.display = name === 'loading' ? 'flex' : 'none';
  D.authScreen.style.display = name === 'auth' ? 'flex' : 'none';
  D.mainScreen.style.display = name === 'main' ? 'flex' : 'none';
...
function updateAvatarUI(el, photoURL, displayName, uid, color) {
  if (!el) return;
  const initials = getInitials(displayName || 'U');
  if (photoURL) {
    el.style.background = 'transparent';
    el.innerHTML = `<img src="${esc(photoURL)}" alt="">`;
  } else {
    el.style.background = color || getUserColor(uid || '');
...
const handleAuthChange = async () => {
  try {
    if (pb.authStore.isValid && pb.authStore.model) {
      ...
      console.log('[ChatNica] User authenticated:', S.user.id, S.user.email);
      try {
        S.profile = await ensureProfile(S.user);
        console.log('[ChatNica] Profile loaded:', S.profile);
      } catch (profileErr) {
        console.error('[ChatNica] Profile load error, using fallback:', profileErr);
        S.profile = {
          id: S.user.id,
          email: S.user.email,
          displayName: S.user.name || S.user.email?.split('@')[0] || 'Usuario',
          photoURL: S.user.avatar || null,
          color: getUserColor(S.user.id),
          bio: '',
          city: '',
          department: '',
        };
